...
NOISE_COLOR = "#444466"  # grey for DBSCAN noise points
...
# ════════════════════════════════════════════════════════════════════════
# MANUAL REGRESSION MODELS (unchanged from v9)
# ════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════
# NEW: POLYNOMIAL REGRESSION (Manual Implementation)
# Generates polynomial features then uses ManualLinearRegression
# ════════════════════════════════════════════════════════════════════════
class ManualPolynomialRegression:
    """
    Polynomial Regression — Manual Implementation.
    Expands input features to degree-d polynomial, then fits with OLS.
    Example: degree=2 with features [x1, x2] → [1, x1, x2, x1², x1·x2, x2²]
    """
    name = "Polynomial"
    ...
    def _expand(self, X):
        ...
        n, p = X.shape
        features = [np.ones((n, 1))]         # intercept term
        feat_names = ["1"]
        ...
        # Higher-degree terms (combinations with repetition)
    ...
    def fit(self, X, y):
        ...
        # Remove the added-intercept column (ManualLinearRegression adds its own)
...
# ════════════════════════════════════════════════════════════════════════
# MANUAL CLASSIFICATION MODELS (unchanged from v9)
# ════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════
# NEW: HIERARCHICAL CLUSTERING — Manual Agglomerative Implementation
# Uses complete-linkage (max distance between cluster members)
# ════════════════════════════════════════════════════════════════════════
class ManualHierarchicalClustering:
    """
    Manual Agglomerative (bottom-up) Hierarchical Clustering.
    Strategy: complete linkage — distance between two clusters =
    maximum distance between any two points across both clusters.
    ...
    def _cluster_distance(self, X, a_members, b_members):
        ...
        if self.linkage_type == "complete":
            return max(dists)           # Furthest-neighbour
        elif self.linkage_type == "single":
            return min(dists)           # Nearest-neighbour
        else:
            return np.mean(dists)       # Average linkage

    def fit(self, X):
        n = len(X)
        # Each point starts as its own cluster {point_index: [members]}
        ...
        while len(clusters) > self.n_clusters:
            ...
            # Merge into the lower-index key
...
# ════════════════════════════════════════════════════════════════════════
# ALGORITHM EXPLAINER DATA  (v9 entries + NEW v10 entries)
# ════════════════════════════════════════════════════════════════════════
EXPLAINER_DATA = {
    # ── Regression (existing) ──────────────────────────────────────────
    "Linear": {
...
# ════════════════════════════════════════════════════════════════════════
# DATA HEALTH SCORE (unchanged from v9)
# ════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════
# PLAIN ENGLISH SUMMARY (unchanged)
# ════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════
# MODEL GRADE, EXPLANATION, TOP-3 FEATURES, WARNINGS, REPORT (unchanged)
# ════════════════════════════════════════════════════════════════════════
...
# ════════════════════════════════════════════════════════════════════════
# MODULE 1 — SMART EDA  (identical to v9, condensed)
# ════════════════════════════════════════════════════════════════════════
...
# ── Hyperparameter controls in a grid ─────────────────────────────────
...
# ════════════════════════════════════════════════════════════════════════
# MODULE 7 — UNSUPERVISED LEARNING & PATTERN DISCOVERY  (EXPANDED v10)
# ════════════════════════════════════════════════════════════════════════
st.markdown(module_banner(
...
if len(numeric_cols) < 2:
    ...
    UNSUP_ALGOS = [
        "K-Means Clustering",
        "DBSCAN — Density-Based Clustering",          # NEW
        "Hierarchical Clustering (Manual)",             # NEW
        "t-SNE 2D Visualization",                      # NEW
    ...
    if len(unsup_feats) < 2:
        ...
        # ════════════════════════════════════════════════════════════════
        # K-MEANS (condensed from v9, identical logic)
        # ════════════════════════════════════════════════════════════════
...
""", unsafe_allow_html=True)
            ...
            with db1:
                db_eps = st.slider("ε (eps) — neighbourhood radius", 0.1, 5.0, 0.5, 0.05, key="db_eps",
                                   help="Max distance between two points to be considered neighbours. Smaller = tighter clusters.")
            with db2:
                db_min = st.slider("min_samples — core point threshold", 2, 30, 5, 1, key="db_min",
                                   help="Min points within eps to form a core point. Higher = fewer, denser clusters.")
            with db3:
            ...
            if st.session_state.get("_db_labels") is not None:
                ...
                # Draw noise points first (grey, behind clusters)
                ...
                # Beginner explanation of result
                st.markdown(f"""
...
""", unsafe_allow_html=True)
            ...
            if run_hc:
                with st.spinner("Running hierarchical clustering…"):
                    ...
                    # sklearn for dendrogram (uses full dataset if small enough)
...
<div class="problem-explain-box" style="border-left-color:#e0a844;">
  ...
  interpretable as real distances. Do not use t-SNE output as input to other models.
</div>
""", unsafe_allow_html=True)
            ...
            with ts1:
                ts_perp = st.slider("Perplexity", 5, 80, 30, 5, key="ts_perp",
                                    help="Balances local vs global structure. Try 5–50. Roughly = neighbourhood size.")
            with ts2:
                ts_iter = st.slider("Iterations", 250, 1000, 500, 250, key="ts_iter",
                                    help="More iterations = more stable result but slower.")
            with ts3:
                ts_lr = st.slider("Learning rate", 10, 1000, 200, 10, key="ts_lr",
                                  help="Controls step size. 10–1000 — default 200 works for most cases.")
            with ts4:
                ts_max_rows = st.slider("Max rows", 100, min(2000, len(data)), min(500, len(data)), 100, key="ts_rows",
                                         help="t-SNE is O(n²) — reduce rows for speed")

            # Optional: colour by an existing column
            ...
            if st.session_state.get("_tsne_2d") is not None:
                ...
                if ts_colour != "None (single colour)" and ts_colour in data.columns:
                    ...
                    if colour_col.dtype == object or colour_col.nunique() <= 15:
                        ...
                        # Continuous colouring with colormap
                ...
                # Offer to apply clustering on top of t-SNE results
                st.markdown("""
<div class="cluster-info-card">
...
""", unsafe_allow_html=True)
                ...
                # If clustering labels exist, overlay them on t-SNE
        ...
        # ════════════════════════════════════════════════════════════════
            ...
            bin_labels_map = {2:["Low","High"],3:["Low","Mid","High"],
                              4:["Low","Med-Low","Med-High","High"],5:["Very Low","Low","Mid","High","Very High"]}
                        ...
                        if not transactions:
                            ...
                            if ap_model.frequent_itemsets_:
                                fi_rows = sorted([{"Itemset":" & ".join(sorted(fs)),"Support":round(s,4)}
                                                  for fs, s in ap_model.frequent_itemsets_.items()], key=lambda x:-x["Support"])
                            ...
                            if ap_model.rules_:
                                for r in ap_model.rules_[:5]:
                                    ...
                                    st.markdown(f'<div class="apriori-rule-row">'
                                                f'<div class="rule-text">{ant_s} → {con_s}</div>'
...
st.markdown(
    ...
    'INTELLIGENT DATA ANALYSIS &amp; ML SYSTEM v10 · '
    '9 REGRESSION MODELS · 7+ CLASSIFICATION MODELS · '
    'DBSCAN · HIERARCHICAL · t-SNE · K-MEANS · APRIORI · '
    'MANUAL &amp; SKLEARN IMPLEMENTATIONS'
    '</div>', unsafe_allow_html=True)