# ════════════════════════════════════════════════════════════════════════
#  Intelligent Data Analysis & ML System  —  v6 (FIXED)
#  FIX: Categorical targets (Result, Genre etc.) now selectable
#  FIX: Categorical features (Genre etc.) show in feature selector
#  FIX: EDA Target Correlation works for both numeric & categorical targets
#  Auto-detects Regression vs Classification from target variable
#  Regression  → Linear · Ridge · Lasso · KNN · Decision Tree · Random Forest
#  Classification → Logistic · KNN · Naive Bayes · Decision Tree · Random Forest
# ════════════════════════════════════════════════════════════════════════
...
/* FIX: Category tag badge in feature selector */
...
st.markdown(
    ...
    'Supports Numeric & Categorical Features/Targets · Manual & Library Models</div>',
    unsafe_allow_html=True,
)
...
all_cols     = data.columns.tolist()   # ← includes BOTH numeric + categorical
...
# ── 1.5 Target Correlation  ← FIXED: supports both numeric & categorical targets
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
st.markdown("## Target Correlation Analysis")

# FIX: allow ALL columns as EDA target (not just numeric_cols)
target_col_eda = st.selectbox(
    "Select Target Column for EDA",
    all_cols,          # ← was: numeric_cols — now includes Result, Genre, etc.
    key="eda_target"
)
...
if eda_target_is_numeric:
    # --- Original numeric-target branch ---
    ...
    # --- FIXED: Categorical target branch (e.g. Result=Hit/Flop, Genre) ---
    ...
    ax_vd.bar(value_counts.index.astype(str), value_counts.values, color=bar_clrs,
              edgecolor=AXES_BG, linewidth=0.5)
    ...
    # Box plots: each numeric feature broken down by target class
    if numeric_cols:
        ...
        for feat in numeric_cols:
            ...
            group_data = [data[data[target_col_eda] == cls][feat].dropna().values
                          for cls in value_counts.index]
            bp = ax_bp2.boxplot(group_data, patch_artist=True,
        ...
        # Point-biserial / ANOVA correlation table for numeric vs categorical
...
def build_summary(data, numeric_cols, cat_cols, total_miss,
    ...
    # FIX: show categorical columns in summary
    if cat_cols:
        items.append(("🏷️", "Categorical Columns",
            f"{len(cat_cols)}: {', '.join(cat_cols)}"))
...
# ── 2.1 Data Config  ← FIXED: all_cols for target, all non-target cols for features
st.markdown("## Data Configuration")

# Info banner about categorical support
if cat_cols:
    cat_list = ", ".join(f"**{c}**" for c in cat_cols)
...
with dc1:
    # FIX: Target can be ANY column — including categorical ones like Result, Genre
    ml_target = st.selectbox(
        "Target Column (Y) — numeric or categorical",
        all_cols,          # ← was: numeric_cols only — now all columns
        key="ml_target"
    )
with dc2:
    avail_feats = [c for c in all_cols if c != ml_target]
    # FIX: default selects ALL available features (including categoricals)
    ml_features = st.multiselect(
        "Input Features (X) — numeric & categorical supported",
        avail_feats,
        default=avail_feats,   # ← default to ALL columns
        key="ml_features"
    )

# Show type tags for selected features
if ml_features:
    tags = ""
...
# ── 2.2 Mode Detection Banner ────────────────────────────────────────────
ml_mode      = detect_mode(data[ml_target])
n_unique_tgt = data[ml_target].nunique()

if ml_mode == "regression":
    st.markdown(
        f'<div class="mode-reg">'
        f'<div class="mode-icon">📈</div>'
        f'<div class="mode-tag">Auto-Detected Mode</div>'
        f'<div class="mode-name">Regression Mode</div>'
        f'<div class="mode-desc">'
        f'Target <strong>{ml_target}</strong> is continuous numeric '
        f'({n_unique_tgt} unique values) — training 6 regression models.'
        f'</div></div>',
        unsafe_allow_html=True,
    )
else:
    ...
    st.markdown(
        f'<div class="mode-cls">'
        f'<div class="mode-icon">🏷️</div>'
        f'<div class="mode-tag">Auto-Detected Mode</div>'
        f'<div class="mode-name">Classification Mode</div>'
        f'<div class="mode-desc">'
        f'Target <strong>{ml_target}</strong> has {n_unique_tgt} unique classes '
        f'[{classes_str}] — training 5 classification models.'
        f'</div></div>',
        unsafe_allow_html=True,
    )
...
if train_btn:
    ...
    with st.spinner(f"Training all models in {ml_mode} mode…"):
        ...
        # ── Prepare X — encode categoricals, impute numerics ─────────
        feature_encoders = {}
        X_parts = []
        for feat in ml_features:
            col = data[feat]
            # FIX: detect categorical by dtype OR object string
        ...
        if ml_mode == "regression":
            ...
            le = LabelEncoder()
            # FIX: convert to string first to handle both object and numeric categorical targets
...
if st.session_state.get("_trained"):
    ...
    # ════════════════════════════════════════════════════════════════════
        ...
        for mname in sorted_names:
            ...
            # FIX: show original class labels in confusion matrix
            cm_data    = ev["CM"]
            ...
            # Classification report — FIX: show original class names not encoded ints
            cr = ev["CR"]
            cr_rows = []
            for label_key, metrics_val in cr.items():
                if isinstance(metrics_val, dict):
                    display_key = label_key
                    # FIX: map encoded integer labels back to original class names
                    try:
    ...
    # FIX: Input widgets — selectbox for categoricals, number_input for numerics
    user_vals     = []
    user_vals_raw = []
    ...
    for i, feat in enumerate(t_feat):
        with inp_cols[i % n_inp]:
            if feat in feat_enc:
                # FIX: categorical feature → show original labels in dropdown
                fe       = feat_enc[feat]
    ...
    if last_shown:
        ...
        if ml_mode == "regression":
            ...
            # FIX: convert encoded int back to original label (e.g. "Hit" / "Flop")
            ...
            if hasattr(snap_model, "predict_proba"):
                ...
                # FIX: show original class names on probability chart
        ...
        # Similar Historical Data Points
...
st.markdown('<div style="text-align:center;padding:14px 0;font-family:JetBrains Mono,monospace;'
            ...
            'INTELLIGENT DATA ANALYSIS &amp; ML SYSTEM v6 · AUTO REGRESSION / CLASSIFICATION · '
