...
# ════════════════════════════════════════════════════════════════════════ # Intelligent Data Analysis & ML System — v10 # New in v10: # SUPERVISED — Regression: # + Support Vector Regression (SVR, sklearn) # + Polynomial Regression (Manual, degree 2/3) # + Gradient Boost


# ════════════════════════════════════════════════════════════════════════
#  Intelligent Data Analysis & ML System  —  v10
#  New in v10:
#    SUPERVISED — Regression:
#      + Support Vector Regression (SVR, sklearn)
#      + Polynomial Regression (Manual, degree 2/3)
...
#      + Hierarchical / Agglomerative Clustering (Manual)
#      + t-SNE 2D Visualization (sklearn)
# ════════════════════════════════════════════════════════════════════════
...
# ────────────────────────────────────────────────────────────────────────
# GLOBAL CSS  (original + landing additions)
# ────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
...
/* Health Score */
.health-score-box { border-radius:10px; padding:20px 26px; margin:14px 0;
...
/* English Summary */
.english-summary-box { background:linear-gradient(135deg,#081828 0%,#060f1c 100%);
    border:1px solid #1a3a54; border-radius:10px; padding:18px 22px; margin:10px 0; }
.english-summary-box .es-title { font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--a1);
    letter-spacing:0.14em; text-transform:uppercase; margin-bottom:12px;
    padding-bottom:8px; border-bottom:1px solid var(--border); }
.english-summary-box .es-item { font-size:0.82rem; color:#a8ccde; padding:5px 0;
...
/* Problem explain */
.problem-explain-box { background:linear-gradient(90deg,#071428 0%,#050e1e 100%);
...
/* Grade banner */
.grade-banner { display:inline-flex; align-items:center; gap:14px;
...
.top3-box { background:#081420; border:1px solid #0f2a3a; border-radius:9px;
    padding:16px 20px; margin:10px 0; }
.top3-title { font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:var(--a1);
    letter-spacing:0.14em; text-transform:uppercase; margin-bottom:12px;
    padding-bottom:8px; border-bottom:1px solid var(--border); }
.top3-item { display:flex; align-items:center; gap:14px;
...
/* Warning boxes */
.warn-box { border-radius:8px; padding:12px 18px; margin:8px 0;
    font-size:0.8rem; line-height:1.7; display:flex; align-items:flex-start; gap:12px; }
...
/* Comparison table */
.compare-table { width:100%; border-collapse:collapse; font-family:'JetBrains Mono',monospace;
    font-size:0.82rem; margin:12px 0; border-radius:8px; overflow:hidden; }
.compare-table th { background:#0a1c2e; color:var(--a1); font-size:0.7rem;
...
/* K-Means styles */
.km-step-header { display:flex; align-items:center; gap:14px;
...
.km-step-num { font-family:'JetBrains Mono',monospace; font-size:1.4rem; font-weight:700;
...
.km-explainer { background:linear-gradient(135deg,#081828 0%,#060f1c 100%);
    border:1px solid #0f2e46; border-radius:10px; padding:20px 26px; margin:10px 0 18px 0; }
...
.km-summary-tile { flex:1; min-width:130px;
...
.km-quality-badge { display:inline-block; font-family:'JetBrains Mono',monospace;
...
.algo-card { background:linear-gradient(135deg,#081828 0%,#060f1c 100%);
    border:1px solid #1a3a54; border-radius:12px; padding:0; margin:14px 0 18px 0;
    overflow:hidden; animation:cardFadeIn 0.4s ease both; }
.algo-card-bar { height:4px; width:100%; }
.algo-card-inner { padding:20px 24px 22px; }
.algo-card-header { display:flex; align-items:center; gap:14px; margin-bottom:14px; }
.algo-card-icon { width:42px; height:42px; border-radius:10px; display:flex; align-items:center;
...
.algo-card-summary { font-size:0.83rem; color:#a0bfd4; line-height:1.7; margin-bottom:16px;
    padding-bottom:14px; border-bottom:1px solid #0f2236; }
.algo-card-grid { display:grid; grid-template-columns:1fr 1fr 1fr; gap:12px; margin-bottom:16px; }
.algo-detail-box { background:#060d18; border:1px solid #0f2236; border-radius:8px; padding:12px 14px; }
.algo-detail-label { font-family:'JetBrains Mono',monospace; font-size:0.64rem; letter-spacing:0.14em;
    text-transform:uppercase; margin-bottom:8px; }
.algo-detail-text { font-size:0.78rem; color:#7ea8c4; line-height:1.65; }
.algo-when-strip { display:flex; align-items:flex-start; gap:10px; background:#060f1c;
    border:1px solid #0f2236; border-radius:8px; padding:11px 14px;
    font-size:0.78rem; color:#7ea8c4; line-height:1.6; }
.algo-pills-row { display:flex; gap:8px; flex-wrap:wrap; margin-bottom:14px; }
.algo-pill { font-family:'JetBrains Mono',monospace; font-size:0.62rem; font-weight:600;
...
/* NEW: Noise badge for DBSCAN */
.noise-badge { display:inline-block; background:#2a0a1a; border:1px solid #6a2050;
    color:#f080c0; font-family:'JetBrains Mono',monospace; font-size:0.72rem;
    padding:4px 12px; border-radius:4px; margin:4px 0; }
/* NEW: Cluster info card */
.cluster-info-card { background:linear-gradient(135deg,#081828 0%,#060f1c 100%);
    border:1px solid #1a3a54; border-radius:8px; padding:14px 18px; margin:8px 0; }
.cluster-info-card .ci-header { font-family:'JetBrains Mono',monospace; font-size:0.78rem;
    color:var(--a2); font-weight:700; margin-bottom:6px; }
...
.landing-hero::before {
    ...
    background: linear-gradient(90deg, transparent, #2d8fcb 30%, #22a878 70%, transparent);
}
.landing-badge {
...
.landing-title .grad {
    background: linear-gradient(135deg, #2d8fcb 0%, #22e8a0 100%);
...
.feat-card {
    background: linear-gradient(150deg, #0c1e32 0%, #08131f 100%);
...
.feat-card:hover {
    ...
    box-shadow: 0 20px 48px rgba(0,0,0,0.6),
                0 0 0 1px var(--card-accent, #2d8fcb),
...
.feat-card-icon {
    ...
    display: block;
    line-height: 1;
    filter: drop-shadow(0 0 14px var(--card-accent, #2d8fcb));
...
.upload-cta-section::before {
    content: '';
    position: absolute;
    bottom: 0; left: 50%; transform: translateX(-50%);
    width: 500px; height: 1px;
    background: linear-gradient(90deg, transparent, #0f2236 50%, transparent);
}
.upload-hint-text {
    font-size: 0.72rem;
    color: #2a5a78;
...
div[data-testid="stButton"].upload-cta-btn > button {
    background: linear-gradient(135deg, #1a5a8a, #0d3a5c) !important;
    ...
    box-shadow: 0 4px 20px rgba(45,143,203,0.2) !important;
}
div[data-testid="stButton"].upload-cta-btn > button:hover {
    background: linear-gradient(135deg, #2d8fcb, #1a6aaa) !important;
    box-shadow: 0 10px 36px rgba(45,143,203,0.45) !important;
...
class ManualPolynomialRegression:
    ...
    def __init__(self, degree=2):
        self.degree = degree
        self._linear = ManualLinearRegression()
        self.beta = None
    ...
    def _expand(self, X):
        ...
        feat_names = ["1"]
        for j in range(p):
            features.append(X[:, j:j+1])
            feat_names.append(f"x{j+1}")
        ...
        for d in range(2, self.degree + 1):
            for combo in combinations_with_replacement(range(p), d):
                ...
                for idx in combo:
                    ...
                    name_parts.append(f"x{idx+1}")
                features.append(term)
                feat_names.append("·".join(name_parts))
        self.feature_names_out_ = feat_names
        return np.hstack(features)

    def fit(self, X, y):
        Xpoly = self._expand(X)
        Xpoly_no_intercept = Xpoly[:, 1:]
        self._linear.fit(Xpoly_no_intercept, y)
        self.beta = self._linear.get_coefficients()

    def predict(self, X):
        Xpoly = self._expand(X)
        Xpoly_no_intercept = Xpoly[:, 1:]
        return self._linear.predict(Xpoly_no_intercept)
...
# ════════════════════════════════════════════════════════════════════════
# UNSUPERVISED — MANUAL IMPLEMENTATIONS
# ════════════════════════════════════════════════════════════════════════
class ManualKMeans:
...
class ManualHierarchicalClustering:
    def __init__(self, n_clusters=3, linkage_type="complete"):
        self.n_clusters   = n_clusters
        self.linkage_type = linkage_type
        self.labels_      = None

    def _cluster_distance(self, X, a_members, b_members):
        dists = [np.linalg.norm(X[i] - X[j])
                 for i in a_members for j in b_members]
        if not dists:
            return np.inf
...
# ════════════════════════════════════════════════════════════════════════
# ALGORITHM EXPLAINER DATA
# ════════════════════════════════════════════════════════════════════════
EXPLAINER_DATA = {
    "Linear": {
        "color": "#2d8fcb", "icon": "📏", "tag": "Regression · Manual",
        "summary": "Finds the best-fit straight line minimising squared errors.",
        "analogy": "A rubber band stretched to sit as close as possible to all data points.",
        "strengths": ["Fast", "Interpretable", "Works well for linear relationships"],
        "weaknesses": ["Assumes linearity", "Sensitive to outliers", "No interactions"],
        "when_to_use": "When you expect a straight-line relationship and need explainability.",
        "pills": [("Interpretable","algo-pill-green"),("Fast","algo-pill-blue"),("Low complexity","algo-pill-blue")],
    },
    "Ridge": {
        "color": "#22a878", "icon": "🏔️", "tag": "Regression · Regularised · Manual",
        "summary": "Linear Regression + L2 penalty to shrink large coefficients.",
        "analogy": "A spring pulling every coefficient back toward zero.",
        "strengths": ["Handles correlated features","Rarely overfits","Interpretable"],
        "weaknesses": ["Keeps all features","Needs λ tuning","Assumes linearity"],
        "when_to_use": "When you have many correlated features and Linear is unstable.",
        "pills": [("Regularised","algo-pill-green"),("Handles collinearity","algo-pill-blue")],
    },
    "Lasso": {
        "color": "#e0a844", "icon": "🔦", "tag": "Regression · Sparse · Manual",
        "summary": "Like Ridge but L1 penalty drives unimportant weights to exactly zero.",
        "analogy": "A hard budget cap — once a coefficient hits zero it stays there.",
        "strengths": ["Built-in feature selection","Sparse output","Works with few important features"],
        "weaknesses": ["Drops correlated features arbitrarily","Slower","λ needs tuning"],
        "when_to_use": "When you suspect only a few features really matter.",
        "pills": [("Feature selector","algo-pill-green"),("Sparse output","algo-pill-blue")],
    },
    "KNN": {
        "color": "#a060c8", "icon": "🗺️", "tag": "Regression & Classification · Non-parametric · Manual",
        "summary": "Predicts by averaging the K nearest training examples.",
        "analogy": "Asking your K nearest neighbours for advice and averaging their answers.",
        "strengths": ["Zero training time","Captures non-linearity","No distribution assumptions"],
        "weaknesses": ["Slow prediction","Bad in high dimensions","Sensitive to K"],
        "when_to_use": "Small-to-medium datasets with local, non-linear patterns.",
        "pills": [("Non-parametric","algo-pill-purple"),("No training","algo-pill-blue"),("Slow predict","algo-pill-amber")],
    },
    "Decision Tree": {
        "color": "#c05555", "icon": "🌳", "tag": "Regression & Classification · Tree · sklearn",
        "summary": "Learns a series of yes/no questions to split data into pure groups.",
        "analogy": "A flowchart of questions splitting the data step by step.",
        "strengths": ["Fully interpretable","Handles mixed types","No preprocessing"],
        "weaknesses": ["Overfits easily","Unstable","Piecewise-constant predictions"],
        "when_to_use": "When interpretability is the top priority.",
        "pills": [("Interpretable","algo-pill-green"),("No preprocessing","algo-pill-blue"),("Overfits","algo-pill-red")],
    },
    "Random Forest": {
        "color": "#d05090", "icon": "🌲", "tag": "Regression & Classification · Ensemble · sklearn",
        "summary": "Hundreds of Decision Trees trained on random subsets — aggregate their votes.",
        "analogy": "100 experts each seeing partial evidence then voting collectively.",
        "strengths": ["High accuracy","Resistant to overfitting","Feature importance"],
        "weaknesses": ["Slow to predict","Black box","Memory-heavy"],
        "when_to_use": "First serious model on any tabular dataset.",
        "pills": [("Ensemble","algo-pill-green"),("High accuracy","algo-pill-green"),("Feature importance","algo-pill-blue")],
    },
    "SVR": {
        "color": "#44c8e0", "icon": "📐", "tag": "Regression · Kernel · sklearn",
        "summary": (
            "Support Vector Regression finds a tube of width ε around the data. "
            "Only points outside the tube (support vectors) influence the model. "
            "Kernels like RBF allow it to model non-linear curves."
        ),
        "analogy": (
            "Drawing the widest possible road through your data — only the cars "
            "that fall off the road's edge matter for positioning the road."
        ),
        "strengths": [
            "Works well in high-dimensional space",
            "Robust to outliers inside the ε-tube",
            "Non-linear patterns via RBF kernel",
        ],
        "weaknesses": [
            "Slow to train on large datasets (> ~10 k rows)",
            "Sensitive to feature scaling — always standardise",
            "Hyperparameters C, ε, γ need tuning",
        ],
        "when_to_use": (
            "Medium-sized datasets where a non-linear relationship exists "
            "and you need robustness to outliers."
        ),
        "pills": [
            ("Kernel trick",   "algo-pill-blue"),
            ("Non-linear",     "algo-pill-green"),
            ("Needs scaling",  "algo-pill-amber"),
            ("Slow on big data","algo-pill-red"),
        ],
    },
    "Polynomial": {
        "color": "#e07030", "icon": "〽️", "tag": "Regression · Manual · Polynomial Features",
        "summary": (
            "Polynomial Regression extends Linear Regression by adding squared, cubed, "
            "and interaction terms as new features, then fitting an OLS line through them. "
            "This lets a linear model capture curves."
        ),
        "analogy": (
            "Instead of fitting a straight ruler, you bend the ruler into a curve "
            "by adding 'how much is x²?' as an extra measurement column."
        ),
        "strengths": [
            "Captures non-linear trends with a simple extension",
            "Still uses the interpretable OLS formula",
            "Manual implementation — full transparency",
        ],
        "weaknesses": [
            "Feature explosion with high degree and many columns",
            "Prone to severe overfitting at degree ≥ 4",
            "Prediction can blow up outside the training range",
        ],
        "when_to_use": (
            "When the relationship is clearly curved (e.g. U-shaped) "
            "but you still want to keep the model simple. Use degree=2 first."
        ),
        "pills": [
            ("Manual OLS",      "algo-pill-green"),
            ("Curved patterns", "algo-pill-blue"),
            ("Overfits at d≥4", "algo-pill-red"),
            ("Feature explosion","algo-pill-amber"),
        ],
    },
    "Gradient Boosting": {
        "color": "#70c040", "icon": "🚀", "tag": "Regression & Classification · Ensemble · sklearn",
        "summary": (
            "Gradient Boosting builds trees one at a time, where each new tree "
            "corrects the errors of all previous trees. The result is an extremely "
            "accurate ensemble that learns from its mistakes step by step."
        ),
        "analogy": (
            "A student who keeps taking the same test and each time studies only "
            "the questions they got wrong last time — they get better and better "
            "with each round."
        ),
        "strengths": [
            "Typically the most accurate algorithm on structured data",
            "Handles mixed feature types natively",
            "Robust to outliers with careful tuning",
        ],
        "weaknesses": [
            "Slow to train (sequential — cannot parallelise)",
            "Many hyperparameters to tune (learning_rate, n_estimators, depth)",
            "Can overfit if not regularised",
        ],
        "when_to_use": (
            "When accuracy is the primary goal and you have enough compute time. "
            "Use after simpler baselines establish a floor."
        ),
        "pills": [
            ("Highest accuracy", "algo-pill-green"),
            ("Boosting ensemble","algo-pill-blue"),
            ("Sequential train", "algo-pill-amber"),
            ("Many HPs",         "algo-pill-amber"),
        ],
    },
    "Logistic": {
        "color": "#2d8fcb", "icon": "📊", "tag": "Classification · Manual",
        "summary": "Estimates class probabilities via softmax; predicts the highest.",
        "analogy": "Weighted voting system converting raw scores to percentage chances.",
        "strengths": ["Fast","Calibrated probabilities","Interpretable"],
        "weaknesses": ["Linear boundary","Struggles with non-linear separation","Needs scaling"],
        "when_to_use": "When you need probabilities and classes are roughly linearly separable.",
        "pills": [("Interpretable","algo-pill-green"),("Outputs probs","algo-pill-blue"),("Linear boundary","algo-pill-amber")],
    },
    "Naive Bayes": {
        "color": "#e0a844", "icon": "🧮", "tag": "Classification · Probabilistic · Manual",
        "summary": "Bayes' theorem assuming all features are independent.",
        "analogy": "A spam filter multiplying per-word probabilities assuming independence.",
        "strengths": ["Extremely fast","Low data needs","Robust"],
        "weaknesses": ["Independence rarely true","Poor with correlated features","No interactions"],
        "when_to_use": "Scarce data, fast baselines, or text classification.",
        "pills": [("Very fast","algo-pill-green"),("Probabilistic","algo-pill-blue"),("Strong independence","algo-pill-red")],
    },
    "SVM": {
        "color": "#44c8e0", "icon": "🏹", "tag": "Classification · Kernel · sklearn",
        "summary": (
            "Support Vector Machine finds the widest margin hyperplane separating classes. "
            "With an RBF kernel it can separate non-linearly separable data by projecting "
            "it into a higher-dimensional space."
        ),
        "analogy": (
            "Drawing the fattest possible line between two groups of dots — the line "
            "is positioned by only the dots right at the edge (support vectors), "
            "not by all the dots in the middle."
        ),
        "strengths": [
            "Very effective in high-dimensional spaces",
            "Robust to outliers (margin-based)",
            "Flexible decision boundary via kernels",
        ],
        "weaknesses": [
            "Slow on large datasets",
            "Requires feature scaling",
            "No direct probability output (needs calibration)",
        ],
        "when_to_use": (
            "Small-to-medium datasets with a complex, non-linear decision boundary. "
            "Especially powerful when there are many features relative to samples."
        ),
        "pills": [
            ("Max margin",     "algo-pill-green"),
            ("Kernel trick",   "algo-pill-blue"),
            ("Needs scaling",  "algo-pill-amber"),
            ("Slow on large",  "algo-pill-red"),
        ],
    },
    "XGBoost": {
        "color": "#e07030", "icon": "⚡", "tag": "Classification · XGBoost · sklearn-compatible",
        "summary": (
            "XGBoost (Extreme Gradient Boosting) is an optimised, parallelised gradient "
            "boosting library. It adds L1/L2 regularisation, built-in cross-validation, "
            "and handles missing values natively, making it a competition favourite."
        ),
        "analogy": (
            "Like Gradient Boosting but with a turbocharger — it runs faster, adds "
            "automatic regularisation, and can handle missing values without preprocessing."
        ),
        "strengths": [
            "Extremely fast (parallelised training)",
            "Regularised — resistant to overfitting",
        ...
        "when_to_use": (
            "Kaggle-style competitions or any tabular classification task where "
            "maximum accuracy is required and training time allows."
        ),
        "pills": [
            ("Fastest boosting","algo-pill-green"),
            ("Regularised",     "algo-pill-blue"),
            ("Handles NaN",     "algo-pill-blue"),
            ("Many HPs",        "algo-pill-amber"),
        ],
    },
}
...
def render_explainer_card(model_name: str, mode: str = "regression") -> None:
    ...
    if data is None:
        st.markdown(
            f'<div class="problem-explain-box">No explainer available for <strong>{model_name}</strong>.</div>',
            unsafe_allow_html=True)
        return
    color = data["color"]
    pills_html = ""
    ...
    def detail_box(label, label_color, content):
        return (f'<div style="background:#060d18;border:1px solid #0f2236;border-radius:8px;padding:12px 14px;">'
                f'<div style="font-family:JetBrains Mono,monospace;font-size:0.64rem;letter-spacing:0.14em;'
                f'text-transform:uppercase;color:{label_color};margin-bottom:8px;">{label}</div>'
                f'<div style="font-size:0.78rem;color:#7ea8c4;line-height:1.65;">{content}</div></div>')
    grid_html = (
...
<style>
  ...
  *{{box-sizing:border-box;margin:0;padding:0;}} body{{background:transparent;font-family:'Inter',sans-serif;}}
  @keyframes cardIn{{from{{opacity:0;transform:translateY(8px)}}to{{opacity:1;transform:translateY(0)}}}}
  .card{{animation:cardIn 0.35s ease both;}} li::before{{content:"› ";color:#2d8fcb;}}
</style></head><body>
<div class="card" style="background:linear-gradient(135deg,#081828 0%,#060f1c 100%);border:1px solid #1a3a54;border-radius:12px;overflow:hidden;margin:4px 0 12px;">
  <div style="height:4px;background:{color};width:100%;"></div>
  <div style="padding:20px 24px 22px;">
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:14px;">
      <div style="width:42px;height:42px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1.3rem;flex-shrink:0;background:{color}22;border:1px solid {color}44;">{data["icon"]}</div>
      <div>
        <div style="font-family:JetBrains Mono,monospace;font-size:1.05rem;font-weight:700;color:#a8ccde;">{model_name}</div>
        <div style="font-family:JetBrains Mono,monospace;font-size:0.65rem;letter-spacing:0.14em;text-transform:uppercase;color:{color};margin-top:3px;">{data["tag"]}</div>
      </div>
    </div>
    <div style="margin-bottom:14px;">{pills_html}</div>
    <div style="font-size:0.83rem;color:#a0bfd4;line-height:1.7;margin-bottom:16px;padding-bottom:14px;border-bottom:1px solid #0f2236;">{data["summary"]}</div>
    {grid_html}
    <div style="display:flex;align-items:flex-start;gap:10px;background:#060f1c;border:1px solid #0f2236;border-radius:8px;padding:11px 14px;font-size:0.78rem;color:#7ea8c4;line-height:1.6;">
      <span style="font-size:1rem;min-width:20px;">💡</span>
      <div><span style="font-family:JetBrains Mono,monospace;font-size:0.68rem;color:#2d8fcb;letter-spacing:0.12em;text-transform:uppercase;">When to use</span><br>{data["when_to_use"]}</div>
    </div>
  </div>
</div></body></html>"""
...
def render_all_explainers(mode: str = "regression") -> None:
    ...
    with st.expander("📚  All Models Explained — click to expand", expanded=False):
        st.markdown('<div style="font-family:JetBrains Mono,monospace;font-size:0.72rem;color:#2d8fcb;'
                    'letter-spacing:0.14em;text-transform:uppercase;margin-bottom:14px;padding-bottom:8px;'
                    'border-bottom:1px solid #0f2236;">⬡ Algorithm Reference — All Models</div>', unsafe_allow_html=True)
...
def km_step(num, title, sub=""):
    ...
    return (f'<div class="km-step-header"><div class="km-step-num">{num}</div>'
            f'<div><div class="km-step-title">{title}</div>{sub_html}</div></div>')
...
def compute_health_score(data, numeric_cols, outlier_counts):
    ...
    miss_penalty = min(30, int(miss_pct * 1.5)); score -= miss_penalty
    ...
    out_penalty    = min(25, int(outlier_pct * 2)); score -= out_penalty
    breakdown["Outliers (IQR)"] = f"-{out_penalty} pts  ({total_outliers} outlier values)"
    dup_count = int(data.duplicated().sum()); dup_pct = dup_count / max(len(data), 1) * 100
    dup_penalty = min(20, int(dup_pct * 2)); score -= dup_penalty
    ...
    skew_penalty = min(25, len(skewed_cols) * 5); score -= skew_penalty
...
def render_health_score(score, breakdown, label, css_class):
    ...
    return f"""<div class="health-score-box {css_class}">
...
def build_english_summary(data, numeric_cols, cat_cols, total_miss,
    ...
    bullets = []
    bullets.append(f"This dataset has <b>{data.shape[0]:,} rows</b> and <b>{data.shape[1]} columns</b> "
                   f"({len(numeric_cols)} numeric, {len(cat_cols)} categorical).")
    if target_col in numeric_cols:
        bullets.append(f"The target column <b>'{target_col}'</b> is <b>numeric</b> — likely a <b>regression</b> problem.")
    else:
        n_cls = data[target_col].nunique() if target_col in data.columns else "?"
        bullets.append(f"The target column <b>'{target_col}'</b> has <b>{n_cls} categories</b> — a <b>classification</b> problem.")
    if corr_with_target is not None and len(corr_with_target):
        best_feat = corr_with_target.abs().idxmax(); r_val = corr_with_target[best_feat]
    ...
    if total_miss == 0:
        ...
        bullets.append(f"There are <b>{total_miss:,} missing values</b>. Worst column: <b>'{worst_col}'</b>.")
    if outlier_counts:
        bad_cols = [c for c, n in outlier_counts.items() if n > 0]
        if bad_cols:
            bullets.append(f"Outliers in <b>{len(bad_cols)} column(s)</b>: {', '.join(f'<b>{c}</b>' for c in bad_cols[:4])}.")
    return bullets

def render_english_summary(bullets):
    items_html = "".join(
        f'<div class="es-item"><span class="es-bullet">›</span><span>{b}</span></div>' for b in bullets)
    return f'<div class="english-summary-box"><div class="es-title">⬡ Plain English Summary</div>{items_html}</div>'

# ════════════════════════════════════════════════════════════════════════
# MODEL GRADE, EXPLANATION, TOP-3 FEATURES, WARNINGS, REPORT
# ════════════════════════════════════════════════════════════════════════
...
def render_grade_banner(model_name, score_val, metric_name="R²"):
    ...
    return f"""<div class="grade-banner grade-{css}">
  <div class="grade-letter">{letter}</div>
  <div class="grade-text"><strong>Model Grade: {letter} — {label}</strong><br>
  {model_name} scored {metric_name} = {score_val:.4f}</div></div>"""

MODEL_EXPLANATIONS = {
    "Random Forest":    "Random Forest combines many decision trees to reduce errors and handles complex non-linear patterns extremely well.",
    "Decision Tree":    "Decision Tree makes decisions by splitting data on the most informative features — easy to understand.",
    "Linear":           "Linear Regression achieved the best score — ideal when the relationship is roughly linear.",
    "Ridge":            "Ridge Regression adds a penalty on large coefficients, preventing overfitting when features are correlated.",
    "Lasso":            "Lasso can automatically set unimportant coefficients to zero, acting as a built-in feature selector.",
    "KNN":              "K-Nearest Neighbours topped the models — intuitive and effective for datasets with clear local structure.",
    "Logistic":         "Logistic Regression was the best classifier — estimates class probabilities, very fast and interpretable.",
    "Naive Bayes":      "Naive Bayes performed best — applies Bayes' theorem assuming feature independence, very fast.",
    "SVR":              "Support Vector Regression found the optimal ε-tube, making it robust to outliers and non-linear patterns.",
    "SVM":              "Support Vector Machine found the widest-margin boundary, handling complex non-linear separations via the RBF kernel.",
    "Polynomial":       "Polynomial Regression captured curved trends by expanding features to higher degrees before fitting.",
    "Gradient Boosting":"Gradient Boosting iteratively corrects its own errors — one of the most accurate algorithms on tabular data.",
    "XGBoost":          "XGBoost (Extreme Gradient Boosting) combines speed, regularisation, and handling of missing values for top accuracy.",
}
...
def render_problem_explanation(mode, target_col, n_unique):
    if mode == "regression":
        body = (f"<strong>This is a Regression problem</strong> because the target column "
                f"<code>{target_col}</code> contains continuous numeric values ({n_unique} unique values).")
    else:
        body = (f"<strong>This is a Classification problem</strong> because the target column "
                f"<code>{target_col}</code> contains discrete categories ({n_unique} unique classes).")
...
def get_top3_features(model, feature_names):
    ...
    if hasattr(model, "feature_importances_"):
        ...
        if len(coeffs) > 1:
            importances = np.abs(coeffs[1:len(feature_names)+1])
    elif hasattr(model, "W") and model.W is not None:
        importances = np.abs(model.W).max(axis=1)
    elif hasattr(model, "coef_"):
        c = model.coef_
        if c.ndim > 1: importances = np.abs(c).mean(axis=0)
        else:          importances = np.abs(c)
...
def render_top3_features(top3):
    if not top3: return ""
    rows = "".join(
        f'<div class="top3-item"><span class="top3-medal">{medal}</span>'
        f'<span class="top3-feat">{feat}</span>'
        f'<span class="top3-score">{score} importance</span></div>'
...
def render_warning(title, body, level="moderate"):
    ...
    return f"""<div class="warn-box warn-{level}">
  <div class="warn-box-icon">{icon}</div>
  <div class="warn-box-body"><div class="warn-box-title">{title}</div>{body}</div>
</div>"""

def data_warnings(data, numeric_cols):
    warnings_html = []
    if len(data) < 100:
        warnings_html.append(render_warning("Small Dataset",
            f"Only {len(data)} rows. Models may not generalise well.", level="critical"))
    miss_pct = data.isnull().sum().sum() / max(data.size, 1) * 100
    if miss_pct > 20:
        warnings_html.append(render_warning("High Missing Values",
            f"{miss_pct:.1f}% of cells are missing. Consider imputation.", level="critical"))
    elif miss_pct > 5:
        warnings_html.append(render_warning("Some Missing Values",
            f"{miss_pct:.1f}% of cells missing. Auto-filled with median/mode.", level="moderate"))
    return warnings_html

def overfitting_warning(gap):
    if gap > 0.15:
        return render_warning("Possible Overfitting",
            f"Train–test gap = {gap:.4f}. Try reducing max_depth or adding more data.", level="moderate")
    return ""

def build_report_text(mode, best_name, best_res, feature_names, top3,
                       health_score, health_label, last_pred=None,
                       last_pred_cls=None, target_col="target"):
    lines = ["="*60, "  INTELLIGENT ML SYSTEM v10 — ANALYSIS REPORT", "="*60,
             f"\nAnalysis Mode   : {mode.capitalize()}",
             f"Target Column   : {target_col}",
             f"Data Health     : {health_score}/100 ({health_label})",
             "\n--- BEST MODEL ---", f"Model           : {best_name}"]
    if mode == "regression":
        lines += [f"R²  (Test)      : {best_res.get('R²','N/A'):.4f}",
                  f"RMSE            : {best_res.get('RMSE','N/A'):.4f}",
                  f"MAE             : {best_res.get('MAE','N/A'):.4f}"]
        grade, glabel, _ = get_model_grade(best_res.get("R²", 0))
    else:
        lines += [f"Accuracy        : {best_res.get('Accuracy','N/A'):.4f}",
                  f"F1 Score        : {best_res.get('F1','N/A'):.4f}"]
        grade, glabel, _ = get_model_grade(best_res.get("F1", 0))
    lines.append(f"Performance Grade: {grade} — {glabel}")
    lines.append("\n--- TOP FEATURES ---")
    lines += [f"  {m}  {f}  ({s})" for m, f, s in top3] if top3 else ["  N/A"]
    lines.append("\n--- FEATURES USED ---")
    lines += [f"  • {f}" for f in feature_names]
...
# ════════════════════════════════════════════════════════════════════════
# SESSION STATE INIT (for landing UI)
# ════════════════════════════════════════════════════════════════════════
...
<div class="landing-hero">
  ...
  <div class="landing-sub">Upload your dataset to unlock powerful AI insights — automatic model selection, deep EDA, clustering, and live predictions in one unified dashboard.</div>
  <div class="landing-stat-strip">
    <div class="landing-stat">
      <div class="landing-stat-num">9+</div>
      <div class="landing-stat-lbl">Regression Models</div>
    </div>
    <div class="landing-stat">
      <div class="landing-stat-num">7+</div>
      <div class="landing-stat-lbl">Classifiers</div>
    </div>
    <div class="landing-stat">
      <div class="landing-stat-num">4</div>
      <div class="landing-stat-lbl">Clustering Algos</div>
    </div>
    <div class="landing-stat">
      <div class="landing-stat-num">Auto</div>
      <div class="landing-stat-lbl">Mode Detection</div>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
CARDS = [
    {
        ...
        "tag": "Module 1",
        "title": "Data Analysis",
        "desc": "Automatic EDA with statistical summaries, correlation matrices, outlier detection via IQR, and a data health score out of 100.",
    ...
    },
    {
        ...
        "tag": "Module 2",
        "title": "ML Models",
        "desc": "Auto-detects regression vs classification. Trains 9+ regression and 7+ classification models simultaneously with one click.",
    ...
    },
    {
        ...
        "tag": "Module 4",
        "title": "Visualization",
        "desc": "Rich diagnostic charts for every model — actual vs predicted, residual plots, confusion matrices, and feature importance bars.",
    ...
    },
    {
        ...
        "desc": "Auto-generated plain-English insights, model grades (A–D), overfitting warnings, and actionable recommendations.",
        "pills": ["Plain English", "Model Grade", "Overfit Alert", "Recs"],
        "accent": "#e0a844",
        "delay": "0.24s",
    },
    {
        ...
        "tag": "Module 3",
        "title": "Performance",
        "desc": "Side-by-side model comparison with R², RMSE, F1, and overfitting gap. Best model auto-selected and highlighted.",
    ...
    },
    {
        ...
        "desc": "Unsupervised suite: K-Means, DBSCAN with noise detection, Hierarchical with dendrogram, t-SNE 2D, and Apriori rules.",
...
for idx, card in enumerate(CARDS):
    ...
    with col:
        ...
        st.markdown(f"""
<div class="feat-card" style="--card-accent:{card['accent']};animation-delay:{card['delay']};">
...
<div class="upload-cta-section">
  <div class="upload-cta-label">Ready to begin your analysis?</div>
</div>
""", unsafe_allow_html=True)

_, cta_col, _ = st.columns([2, 1.6, 2])
with cta_col:
...
st.markdown("""
<p style="text-align:center;font-family:'JetBrains Mono',monospace;font-size:0.68rem;
color:#1e4a68;letter-spacing:0.1em;margin-top:10px;">
Supports CSV files &nbsp;·&nbsp; All processing is local &nbsp;·&nbsp; No data leaves your browser
</p>
""", unsafe_allow_html=True)
...
# Once "Upload Dataset" is clicked, show the uploader and all ML logic below
st.markdown(
    '<div class="hero-title">Intelligent Data Analysis & ML System</div>',
    unsafe_allow_html=True)
st.markdown(
    '<div class="hero-sub">v10 · Auto-Detects Regression vs Classification · '
    '9 Regression Models · 7+ Classification Models · '
    'DBSCAN · Hierarchical · t-SNE · Manual & Library Implementations</div>',
    unsafe_allow_html=True)
st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)
...
with cr:
    ...
    mv = pd.DataFrame({"Column": data.columns, "Missing": data.isnull().sum().values,
...
if len(numeric_cols) >= 2:
    ...
    sns.heatmap(corr_matrix, annot=True, fmt=".2f",
                ...
                center=0, vmin=-1, vmax=1, square=True, linewidths=0.4, linecolor=GRID_CLR,
                annot_kws={"size": 8, "color": "#c9d8e8"}, cbar_kws={"shrink": 0.8}, ax=ax_c)
    ...
    for i, a in enumerate(numeric_cols):
        for j, b in enumerate(numeric_cols):
            ...
            if r > 0.7:
                st.markdown(f'<div class="insight-pos">📈 {a} ↔ {b} (r = {r:.2f})</div>', unsafe_allow_html=True); found = True
            elif r < -0.7:
                st.markdown(f'<div class="insight-neg">📉 {a} ↔ {b} (r = {r:.2f})</div>', unsafe_allow_html=True); found = True
    if not found:
...
if eda_target_is_numeric:
    ...
    if other_cols and corr_matrix is not None and target_col_eda in corr_matrix.columns:
        ...
        ax_t.set_xlabel("Correlation with Target"); ax_t.set_title(f"Feature Correlations with '{target_col_eda}'")
    ...
    n_classes = len(value_counts); palette = [ACCENT1, ACCENT2, ACCENT3, ACCENT4, ACCENT5, ACCENT6]
    fig_vd, ax_vd = plt.subplots(figsize=(max(5, n_classes*1.2), 3.5))
    bar_clrs = [palette[i % len(palette)] for i in range(len(value_counts))]
    ax_vd.bar(value_counts.index.astype(str), value_counts.values, color=bar_clrs, edgecolor=AXES_BG)
    ...
    ax_vd.set_title(f"Class Distribution — '{target_col_eda}'"); ax_vd.set_ylabel("Count")
    apply_theme(fig_vd, ax_vd); fig_vd.tight_layout(); st.pyplot(fig_vd); plt.close()
    ...
    for row_i in range(int(np.ceil(len(numeric_cols)/2))):
        ...
        for ci in range(2):
            ...
            with st_cols[ci]:
                ...
                apply_theme(fig_bp3, ax_bp3); fig_bp3.tight_layout(pad=1.2); st.pyplot(fig_bp3); plt.close()
                is_bad = n_out > 0; badge_cls = "outlier-found" if is_bad else "outlier-clean"
...
def build_summary(data, numeric_cols, cat_cols, total_miss, corr_with_target, outlier_counts, stats_rows):
    ...
    if total_miss == 0: items.append(("✅", "Missing Values", "Complete — no missing cells detected"))
        ...
        items.append(("⚠", "Missing Values", f"{total_miss:,} missing ({round(total_miss/data.size*100,2)}%) — worst: '{top_m}'"))
    if corr_with_target is not None and len(corr_with_target):
        best = corr_with_target.abs().idxmax(); r = corr_with_target[best]
        items.append(("🔗", "Strongest Predictor", f"'{best}' — {'positive' if r>0 else 'negative'} (r = {r:.4f})"))
    if outlier_counts:
    ...
    items.append(("🔁", "Duplicate Rows", f"{n_dup} duplicate row(s) detected" if n_dup else "None detected"))
    return items
...
bullets = build_english_summary(data, numeric_cols, cat_cols, total_miss,
...
st.markdown(module_banner(
    ...
    "Auto-detects Regression or Classification · 9 Regression + 7 Classification Models"
), unsafe_allow_html=True)

st.markdown("## Data Configuration")
if cat_cols:
    st.info(f"🏷️ Categorical columns: {', '.join(cat_cols)}. Auto label-encoded.")

dc1, dc2 = st.columns(2)
with dc1:
    ml_target = st.selectbox("Target Column (Y)", all_cols, key="ml_target")
with dc2:
    avail_feats = [c for c in all_cols if c != ml_target]
    ml_features = st.multiselect("Input Features (X)", avail_feats, default=avail_feats, key="ml_features")

if ml_features:
    tags = "".join(
        f'<span style="background:#1a0d28;border:1px solid #5a2a7a;color:#c890f0;font-size:0.65rem;padding:2px 7px;border-radius:3px;font-family:JetBrains Mono,monospace;margin:2px;">{f} 🏷️ cat</span> '
        if f in cat_cols else
        f'<span style="background:#0a1a2a;border:1px solid #1a4060;color:#6dc8f0;font-size:0.65rem;padding:2px 7px;border-radius:3px;font-family:JetBrains Mono,monospace;margin:2px;">{f} # num</span> '
...
with oc2: apply_std = st.checkbox("Apply Z-score Standardization", value=True,
                                   help="Recommended — especially for SVR, SVM, KNN, Logistic")
...
if ml_mode == "regression":
    st.markdown(
        ...
        f'({n_unique_tgt} unique values) — training 9 regression models.</div></div>',
        unsafe_allow_html=True)
else:
    ...
    st.markdown(
        ...
        f'<div class="mode-desc">Target <strong>{ml_target}</strong> has {n_unique_tgt} classes '
        f'[{classes_str}] — training {"8" if XGBOOST_AVAILABLE else "7"} classification models.</div></div>',
        unsafe_allow_html=True)
...
with hp_cols[1]:
    st.markdown('<div class="model-config-box"><div class="model-config-title">SVR / SVM</div>', unsafe_allow_html=True)
    svm_c = st.slider("Regularisation C", 0.01, 10.0, 1.0, 0.01, key="svm_c")
    st.markdown('</div>', unsafe_allow_html=True)
with hp_cols[2]:
    st.markdown('<div class="model-config-box"><div class="model-config-title">Polynomial Regression</div>', unsafe_allow_html=True)
    poly_degree = st.slider("Degree", 2, 3, 2, 1, key="poly_degree")
    st.markdown('</div>', unsafe_allow_html=True)
with hp_cols[3]:
    st.markdown('<div class="model-config-box"><div class="model-config-title">Gradient Boosting</div>', unsafe_allow_html=True)
    gb_n = st.slider("N estimators", 50, 300, 100, 50, key="gb_n")
    gb_lr = st.slider("Learning rate", 0.01, 0.3, 0.1, 0.01, key="gb_lr")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with st.spinner(f"Training all models in {ml_mode} mode…"):
        ...
        if ml_mode == "regression":
            models = {
                ...
                "SVR":                SVR(C=svm_c, kernel="rbf", epsilon=0.1),
                "Polynomial":         ManualPolynomialRegression(degree=poly_degree),
                "Gradient Boosting":  GradientBoostingRegressor(n_estimators=gb_n, learning_rate=gb_lr,
            ...
            cls_models_dict = {
                ...
                "Gradient Boosting":  GradientBoostingClassifier(n_estimators=gb_n, learning_rate=gb_lr,
                                                                   max_depth=4, random_state=42),
            }
            if XGBOOST_AVAILABLE:
                cls_models_dict["XGBoost"] = XGBClassifier(
                    n_estimators=gb_n, learning_rate=gb_lr, max_depth=4,
...
if st.session_state.get("_trained"):
    ...
    if ml_mode == "regression":
        ...
        ax_cmp.tick_params(axis="x", rotation=20, labelsize=8)
        ...
        for mname in sorted_names:
            ...
            if hasattr(m_obj, "feature_importances_"):
                fi_data = m_obj.feature_importances_
                fi_title = f"{mname} — Feature Importance (Gini)"
            elif hasattr(m_obj, "get_coefficients") and m_obj.get_coefficients() is not None:
                coeffs = m_obj.get_coefficients()
                fi_data = np.abs(coeffs[1:len(t_feat)+1])
                fi_title = f"{mname} — Feature Importance (|coefficient|)"
            elif hasattr(m_obj, "coef_"):
                c = m_obj.coef_
                fi_data = np.abs(c).flatten()[:len(t_feat)]
                fi_title = f"{mname} — Feature Importance (|coefficient|)"
            ...
            if fi_data is not None and len(fi_data) == len(t_feat):
                ...
                ax_fi.set_title(fi_title); apply_theme(fig_fi, ax_fi)
        ...
        for mname, ev in results.items():
            ...
            comp_rows.append({"Model": mname, "Accuracy": round(ev["Accuracy"],4),
                               "Precision": round(ev["Precision"],4), "Recall": round(ev["Recall"],4),
                               "F1 Score": round(ev["F1"],4), "Train Acc": round(ev["Acc_train"],4),
                               "Overfit Gap": round(gap,4),
                               "Grade": get_model_grade(ev["F1"])[0]})
        ...
        sorted_names = comp_df["Model"].tolist()
        bar_colors   = [CLS_MODEL_COLORS.get(m, ACCENT5) for m in sorted_names]
        edge_colors  = ["#ffd700" if m == best_name else AXES_BG for m in sorted_names]
        edge_widths  = [2.0 if m == best_name else 0.5 for m in sorted_names]
        ...
        for ax, (metric_name, vals) in zip(axes_4, metrics_to_plot):
            ...
            ax.tick_params(axis="x", rotation=20, labelsize=7); apply_theme(fig_4, ax)
        fig_4.suptitle("Classification Model Comparison", fontsize=11, color="#7eb8d4")
        fig_4.tight_layout(); st.pyplot(fig_4); plt.close()

        st.markdown(module_banner("4", "Per-Model Diagnostics",
            "Metrics · Confusion Matrix · Classification Report"), unsafe_allow_html=True)
        for mname in sorted_names:
            ev      = results[mname]
            lc      = CLS_MODEL_COLORS.get(mname, ACCENT5)
            is_best = mname == best_name
            ...
            cm_data = ev["CM"]
            disp_labels = ([str(le.inverse_transform([c])[0]) for c in range(len(classes))]
                           if le else [str(c) for c in range(len(cm_data))])
            ...
            sns.heatmap(cm_data, annot=True, fmt="d",
                        ...
                        linewidths=0.5, linecolor=GRID_CLR, annot_kws={"size":10,"color":"#e8d8f8"},
    ...
    st.markdown(module_banner("6", "Prediction & Advanced Analytics",
        f"{sub6} · What-If · Similar Points · Insights"), unsafe_allow_html=True)
    ...
    user_vals = []; user_vals_raw = []
    n_inp = min(3, len(t_feat)); inp_cols = st.columns(n_inp)
    for i, feat in enumerate(t_feat):
        with inp_cols[i % n_inp]:
            if feat in feat_enc:
                fe = feat_enc[feat]; cat_opts = list(fe.classes_)
                sel = st.selectbox(f"{feat} 🏷️", options=cat_opts, key=f"pred_{feat}")
                user_vals.append(float(fe.transform([sel])[0])); user_vals_raw.append(sel)
            else:
                lo_v = float(dsnap[feat].min()); hi_v = float(dsnap[feat].max())
                mean_v = float(dsnap[feat].mean())
                v = st.number_input(feat, min_value=lo_v, max_value=hi_v, value=mean_v,
                                    step=max((hi_v-lo_v)/200, 1e-6), format="%.4f", key=f"pred_{feat}")
                user_vals.append(v); user_vals_raw.append(v)
    ...
    if do_pred:
        ...
        if ml_mode == "regression":
            ...
            st.session_state["_last_pred"] = pred_val; st.session_state["_last_pred_cls"] = None
        else:
            pred_cls = chosen_model.predict(arr_std)[0]
            pred_prob = None
            if hasattr(chosen_model, "predict_proba"):
                pred_prob = float(chosen_model.predict_proba(arr_std)[0].max())
    ...
    if last_shown:
        ...
        if ml_mode == "regression":
            ...
            ci_lo = pred - 1.96*snap_rmse; ci_hi = pred + 1.96*snap_rmse
            st.markdown(f'<div class="pred-box"><div class="pred-label">Predicted {t_tgt}</div>'
                        f'<div class="pred-val">{pred:.4f}</div>'
                        f'<div class="pred-ci">95% CI: [{ci_lo:.4f}, {ci_hi:.4f}]</div>'
                        f'<div class="pred-model">Model: {snap_model_nm}</div></div>', unsafe_allow_html=True)
        else:
            ...
            st.markdown(f'<div class="pred-box-cls"><div class="pred-label">Predicted Class — {t_tgt}</div>'
            ...
            if hasattr(snap_model, "predict_proba"):
                ...
                proba_labels = ([str(le.inverse_transform([c])[0]) for c in range(len(proba_arr))]
                                if le else [str(c) for c in range(len(proba_arr))])
        ...
        if hasattr(snap_model, "get_coefficients") and snap_model.get_coefficients() is not None:
            ...
            for i, feat in enumerate(t_feat):
                contribs[feat] = float(np.abs(snap_model.W[i]).max()) * float(user_vals_snap[i])
        elif hasattr(snap_model, "feature_importances_"):
            fi = snap_model.feature_importances_
            diff = (last_pred if ml_mode == "regression" and last_pred is not None else 1.0)
            for i, feat in enumerate(t_feat):
            ...
            for i, feat in enumerate(t_feat):
                contribs[feat] = float(c[i]) * float(user_vals_snap[i]) if i < len(c) else 0.0
        else:
        ...
        ax_ex.axvline(0, color=TEXT_CLR, linewidth=0.8); ax_ex.set_title("Feature Contributions")
        ...
        for i, feat in enumerate(t_feat):
            with wi_cols[i % min(3, len(t_feat))]:
                if feat in feat_enc:
                    ...
                    wi_sel  = st.selectbox(f"{feat} 🏷️", options=cat_opts, index=cat_opts.index(cur_sel), key=f"wi_{feat}")
                    wi_vals.append(float(fe.transform([wi_sel])[0]))
                else:
                    lo_v = float(dsnap[feat].min()); hi_v = float(dsnap[feat].max())
                    wv = st.slider(feat, min_value=lo_v, max_value=hi_v, value=float(user_vals_snap[i]),
        ...
        if ml_mode == "regression":
            ...
            st.markdown(f"""<table class="compare-table"><thead><tr><th>Scenario</th><th>Prediction</th><th>Difference</th></tr></thead>
<tbody><tr class="compare-orig"><td>🔵 Original inputs</td><td>{last_pred:.4f}</td><td>—</td></tr>
<tr class="compare-mod"><td>🟢 Modified inputs</td><td>{wi_pred:.4f}</td><td style="color:{dc};">{di} {abs(delta):.4f} ({delta_pct:+.1f}%)</td></tr></tbody></table>""", unsafe_allow_html=True)
        else:
            ...
            ch_color = "#e07070" if changed else "#34d490"
            st.markdown(f"""<table class="compare-table"><thead><tr><th>Scenario</th><th>Predicted Class</th><th>Status</th></tr></thead>
<tbody><tr class="compare-orig"><td>🔵 Original</td><td>{orig_label}</td><td>—</td></tr>
<tr class="compare-mod"><td>🟢 Modified</td><td>{wi_label}</td><td style="color:{ch_color};">{"⚠ Changed!" if changed else "✓ Same"}</td></tr></tbody></table>""", unsafe_allow_html=True)

        st.markdown('<div class="section-divider"></div>', unsafe_allow_html=True)

    st.markdown('<div class="module-banner" style="border-left-color:#e0a844;">'
                '<div class="mod-label" style="color:#e0a844;">⬡ Export</div>'
                '<div class="mod-title">Download Analysis Report</div></div>', unsafe_allow_html=True)
    top3_best = get_top3_features(fitted[best_name], t_feat)
    report_text = build_report_text(
        mode=ml_mode, best_name=best_name, best_res=best_res, feature_names=t_feat,
        top3=top3_best, health_score=st.session_state.get("_health_score", 0),
        health_label=st.session_state.get("_health_label","Unknown"),
        last_pred=st.session_state.get("_last_pred"),
        last_pred_cls=st.session_state.get("_last_pred_cls"), target_col=t_tgt)
    if ml_mode == "regression":
        metrics_df = pd.DataFrame([{"Model": m, "R2_Test": round(r["R²"],4),
            "R2_Train": round(r["r2_train"],4), "RMSE": round(r["RMSE"],4),
            "MAE": round(r["MAE"],4), "Grade": get_model_grade(r["R²"])[0]} for m, r in results.items()])
    else:
        metrics_df = pd.DataFrame([{"Model": m, "Accuracy": round(r["Accuracy"],4),
            "Precision": round(r["Precision"],4), "Recall": round(r["Recall"],4),
            "F1": round(r["F1"],4), "Grade": get_model_grade(r["F1"])[0]} for m, r in results.items()])
    col_dl1, col_dl2 = st.columns(2)
    with col_dl1:
        st.download_button("📄  Download Text Report (.txt)", report_text.encode("utf-8"),
                           "ml_analysis_report.txt", "text/plain", use_container_width=True)
    with col_dl2:
        csv_buffer = io.StringIO(); metrics_df.to_csv(csv_buffer, index=False)
        st.download_button("📊  Download Metrics CSV (.csv)", csv_buffer.getvalue().encode("utf-8"),
                           "ml_metrics.csv", "text/csv", use_container_width=True)
...
st.markdown(module_banner(
    ...
    "K-Means · DBSCAN · Hierarchical Clustering · t-SNE Visualization · Apriori Rules"
), unsafe_allow_html=True)
    ...
    if len(unsup_feats) < 2:
        ...
        if unsup_algo == "K-Means Clustering":
            st.markdown('<div class="module-banner" style="border-left-color:#22e8a0;">'
                        ...
                        '<div class="mod-title">K-Means Clustering</div>'
                        '<div class="mod-sub">Manual implementation — groups data into K clusters by minimising inertia</div>'
            ...
            show_elbow = st.checkbox("Show Elbow Chart", value=True, key="km_elbow")
            ...
            if run_km:
                with st.spinner("Running K-Means…"):
                    ...
                    km_labels = km_model.labels_; km_sil = silhouette_score_manual(X_unsup, km_labels)
                    st.session_state.update({"_km_labels": km_labels, "_km_centroids": km_model.centroids_,
                                             "_km_model": km_model, "_km_sil": km_sil, "_km_k": km_k})
            ...
            if st.session_state.get("_km_labels") is not None:
                ...
                km_sil = st.session_state["_km_sil"]; km_k_used = st.session_state["_km_k"]
                ...
                st.markdown(f'<div class="km-explainer"><p>K-Means divided your <strong>{n_points:,} records</strong> '
                            f'into <strong>{km_k_used} groups</strong> based on: {", ".join(f"<em>{f}</em>" for f in unsup_feats)}. '
                            f'Quality: <span class="{q_cls} km-quality-badge">{q_label} (sil={km_sil:.2f})</span></p></div>',
                            unsafe_allow_html=True)
                ...
                if show_elbow:
                    ...
                    inertias = [ManualKMeans(k=ek, max_iter=200, random_state=42).fit(X_unsup).inertia_ for ek in elbow_ks]
                    fig_elb, ax_elb = plt.subplots(figsize=(7, 3.8))
                    ax_elb.plot(list(elbow_ks), inertias, color=ACCENT7, linewidth=2.5, marker="o", markersize=9, markerfacecolor=ACCENT4, markeredgecolor=AXES_BG)
                    ax_elb.axvline(km_k_used, color=ACCENT3, linewidth=2, linestyle="--", label=f"K={km_k_used}")
                    ax_elb.set_xlabel("K"); ax_elb.set_ylabel("Inertia"); ax_elb.set_title("Elbow Chart")
                ...
                for v_s, l_s in [(str(km_k_used),"Groups"), (f"{n_points:,}","Records"),
                                  (f"{km_sil:.2f}" if not np.isnan(km_sil) else "N/A","Silhouette")]:
            ...
            st.markdown('<div class="module-banner" style="border-left-color:#d05090;">'
                        '<div class="mod-label" style="color:#d05090;">⬡ Algorithm · DBSCAN · sklearn</div>'
                        '<div class="mod-title">DBSCAN — Density-Based Spatial Clustering</div>'
                        '<div class="mod-sub">Finds clusters of arbitrary shape · Automatically labels noise points as outliers</div>'
                        '</div>', unsafe_allow_html=True)

            st.markdown("""
<div class="problem-explain-box" style="border-left-color:#d05090;">
  <strong>How DBSCAN works:</strong><br>
  DBSCAN groups points that are <em>close together</em> (within distance <code>eps</code>) and
  have enough neighbours (<code>min_samples</code>). Unlike K-Means, you don't need to specify
  the number of clusters — DBSCAN discovers it automatically. Points that don't belong to any
  dense region are labelled as <em>noise</em> (shown in grey). Great for detecting outliers!
</div>
""", unsafe_allow_html=True)

            db1, db2, db3 = st.columns(3)
            with db1:
                db_eps = st.slider("ε (eps) — neighbourhood radius", 0.1, 5.0, 0.5, 0.05, key="db_eps",
                                   help="Max distance between two points to be considered neighbours.")
            with db2:
                db_min = st.slider("min_samples — core point threshold", 2, 30, 5, 1, key="db_min",
                                   help="Min points within eps to form a core point.")
            with db3:
                db_metric = st.selectbox("Distance metric", ["euclidean","manhattan","cosine"], key="db_metric")

            run_db = st.button("▶  Run DBSCAN", use_container_width=True, key="run_db")

            if run_db:
                with st.spinner("Running DBSCAN…"):
                    db_model  = DBSCAN(eps=db_eps, min_samples=db_min, metric=db_metric)
                    db_labels = db_model.fit_predict(X_unsup)
                    st.session_state["_db_labels"] = db_labels
                    st.session_state["_db_eps"]    = db_eps
                    st.session_state["_db_min"]    = db_min

            if st.session_state.get("_db_labels") is not None:
                db_labels = st.session_state["_db_labels"]
                unique_clusters = sorted(set(db_labels))
                n_clusters  = len([c for c in unique_clusters if c != -1])
                n_noise     = int((db_labels == -1).sum())
                n_points    = len(db_labels)
                db_sil      = silhouette_score_manual(X_unsup, db_labels) if n_clusters >= 2 else float("nan")

                mc = '<div class="card-grid">'
                for v, l in [(str(n_clusters), "Clusters Found"), (str(n_noise), "Noise Points"),
                             (str(n_points), "Total Records"),
                             (f"{db_sil:.2f}" if not np.isnan(db_sil) else "N/A", "Silhouette Score")]:
                ...
                if noise_pct > 30:
                    st.markdown(render_warning("High Noise",
                        f"{noise_pct:.1f}% of points labelled as noise. Try increasing eps or reducing min_samples.",
                        level="moderate"), unsafe_allow_html=True)
                elif n_clusters == 0:
                    st.markdown(render_warning("No Clusters Found",
                        "All points are noise! Increase eps or reduce min_samples significantly.",
                        level="critical"), unsafe_allow_html=True)

                if X_unsup.shape[1] == 2:
                    plot_X = X_unsup; ax_lx = unsup_feats[0]; ax_ly = unsup_feats[1]
                else:
                    pca_db = ManualPCA(2); plot_X = pca_db.fit_transform(X_unsup)
                    vr = pca_db.explained_variance_ratio_
                    ax_lx = f"PC1 ({vr[0]*100:.1f}%)"; ax_ly = f"PC2 ({vr[1]*100:.1f}%)"

                fig_db, ax_db = plt.subplots(figsize=(10, 6))
                noise_mask = db_labels == -1
                if noise_mask.sum() > 0:
                    ax_db.scatter(plot_X[noise_mask,0], plot_X[noise_mask,1],
                                  s=25, color=NOISE_COLOR, alpha=0.5, marker="x",
                                  linewidths=1.2, label=f"Noise / Outliers (n={noise_mask.sum()})", zorder=2)

                for cid in [c for c in unique_clusters if c != -1]:
                    mask = db_labels == cid; clr = CLUSTER_PALETTE[cid % len(CLUSTER_PALETTE)]
                    ax_db.scatter(plot_X[mask,0], plot_X[mask,1],
                                  s=55, color=clr, alpha=0.8, edgecolors="none",
                                  label=f"Cluster {cid+1} (n={mask.sum():,})", zorder=3)

                ax_db.set_xlabel(ax_lx); ax_db.set_ylabel(ax_ly)
                ax_db.set_title(f"DBSCAN — {n_clusters} Clusters, {n_noise} Noise Points  (ε={db_eps}, min={db_min})", fontsize=12)
                ax_db.legend(fontsize=8.5, framealpha=0.2, labelcolor=TEXT_CLR, facecolor=AXES_BG, edgecolor=GRID_CLR)
                apply_theme(fig_db, ax_db); fig_db.tight_layout(); st.pyplot(fig_db); plt.close()

                if n_clusters > 0:
                    st.markdown("#### Cluster Profiles")
                    profile_df = data[unsup_feats].copy()
                    profile_df["__cluster__"] = db_labels
                    cluster_means = profile_df[profile_df["__cluster__"] != -1].groupby("__cluster__")[unsup_feats].mean().round(3)
                ...
                if n_noise > 0:
                    with st.expander(f"🔴 View {min(n_noise, 20)} Noise / Outlier Records"):
                        noise_df = data[unsup_feats].iloc[noise_mask].head(20).copy().reset_index(drop=True)
                        st.dataframe(noise_df, use_container_width=True)

                st.markdown(f"""
<div class="cluster-info-card">
  <div class="ci-header">📊 What the results mean</div>
  <div class="ci-body">
    DBSCAN found <strong>{n_clusters} dense group(s)</strong> in your data.
    {'No noise points were detected — all records belong to a cluster.' if n_noise == 0 else f'<strong>{n_noise} point(s) ({noise_pct:.1f}%)</strong> did not fit into any cluster and are labelled as <em>noise (outliers)</em>.'}
    {'The silhouette score of ' + f'{db_sil:.2f}' + ' indicates ' + ('strong' if db_sil > 0.5 else 'moderate' if db_sil > 0.25 else 'weak') + ' cluster separation.' if not np.isnan(db_sil) else ''}
  </div>
</div>
""", unsafe_allow_html=True)

        elif unsup_algo == "Hierarchical Clustering (Manual)":
            st.markdown('<div class="module-banner" style="border-left-color:#44c8e0;">'
                        '<div class="mod-label" style="color:#44c8e0;">⬡ Algorithm · Hierarchical · Manual + sklearn</div>'
                        '<div class="mod-title">Hierarchical (Agglomerative) Clustering</div>'
                        '<div class="mod-sub">Manual implementation for small datasets · sklearn dendrogram for visualisation</div>'
                        '</div>', unsafe_allow_html=True)

            st.markdown("""
<div class="problem-explain-box" style="border-left-color:#44c8e0;">
  <strong>How Hierarchical Clustering works:</strong><br>
  Start with every point as its own group. Then repeatedly <em>merge</em> the two closest groups
  until only the desired number remain. The result is a tree (dendrogram) showing how groups
  split apart — you can cut it at any level to get different numbers of clusters.
  This manual implementation uses <em>complete linkage</em> (distance = furthest pair).
</div>
""", unsafe_allow_html=True)

            hc1, hc2, hc3 = st.columns(3)
            with hc1:
                hc_n_clusters = st.slider("Number of Clusters", 2, 10, 3, 1, key="hc_n")
            with hc2:
                hc_linkage = st.selectbox("Linkage type", ["complete","single","average"], key="hc_link",
                                           help="Complete = max distance (tight clusters), Single = min distance (chain), Average = mean")
            with hc3:
                hc_max_rows = st.slider("Max rows for manual algo", 50, min(500, len(data)), min(200, len(data)), 50, key="hc_rows",
                                         help="Manual implementation is O(n³) — limit rows for speed")
            ...
            if run_hc:
                with st.spinner("Running hierarchical clustering…"):
                    X_hc = X_unsup[:hc_max_rows]
                    hc_manual = ManualHierarchicalClustering(n_clusters=hc_n_clusters, linkage_type=hc_linkage)
                    hc_labels_manual = hc_manual.fit_predict(X_hc)

                    X_dend = X_unsup[:min(len(X_unsup), 300)]
                    link_matrix = linkage(X_dend, method=hc_linkage)

                    st.session_state["_hc_labels"] = hc_labels_manual
                    st.session_state["_hc_link_matrix"] = link_matrix
                    st.session_state["_hc_n"]      = hc_n_clusters
                    st.session_state["_hc_X"]      = X_hc

            if st.session_state.get("_hc_labels") is not None:
                hc_labels    = st.session_state["_hc_labels"]
                link_matrix  = st.session_state["_hc_link_matrix"]
                hc_k_used    = st.session_state["_hc_n"]
                X_hc_plot    = st.session_state["_hc_X"]
                uniq_hc, cnt_hc = np.unique(hc_labels, return_counts=True)
                n_hc = len(hc_labels)
                hc_sil = silhouette_score_manual(X_hc_plot, hc_labels) if hc_k_used >= 2 else float("nan")

                mc = '<div class="card-grid">'
                for v, l in [(str(hc_k_used), "Clusters"), (str(n_hc), "Records Used"),
                             (f"{hc_sil:.2f}" if not np.isnan(hc_sil) else "N/A", "Silhouette"),
                             (hc_linkage.capitalize(), "Linkage")]:
                    mc += metric_card(v, l, "unsup-metric-card")
                mc += '</div>'
                st.markdown(mc, unsafe_allow_html=True)
                ...
                with vis1:
                    if X_hc_plot.shape[1] == 2:
                        ...
                        vr = pca_hc.explained_variance_ratio_
                        lx = f"PC1 ({vr[0]*100:.1f}%)"; ly = f"PC2 ({vr[1]*100:.1f}%)"
                    fig_hcs, ax_hcs = plt.subplots(figsize=(6, 5))
                    for cid, cnt in zip(uniq_hc, cnt_hc):
                        mask = hc_labels == cid; clr = CLUSTER_PALETTE[cid % len(CLUSTER_PALETTE)]
                        ax_hcs.scatter(plot_X_hc[mask,0], plot_X_hc[mask,1],
                                       s=55, color=clr, alpha=0.8, edgecolors="none",
                                       label=f"Cluster {cid+1} (n={cnt})")
                    ax_hcs.set_xlabel(lx); ax_hcs.set_ylabel(ly)
                    ax_hcs.set_title(f"Hierarchical — {hc_k_used} Clusters", fontsize=11)
                    ax_hcs.legend(fontsize=8, framealpha=0.2, labelcolor=TEXT_CLR, facecolor=AXES_BG, edgecolor=GRID_CLR)
                    apply_theme(fig_hcs, ax_hcs); fig_hcs.tight_layout(); st.pyplot(fig_hcs); plt.close()

                with vis2:
                    fig_dnd, ax_dnd = plt.subplots(figsize=(6, 5))
                    dendrogram(link_matrix, ax=ax_dnd, no_labels=True,
                               color_threshold=link_matrix[-hc_k_used+1, 2],
                               above_threshold_color=TEXT_CLR,
                               link_color_func=lambda k: CLUSTER_PALETTE[k % len(CLUSTER_PALETTE)])
                    ax_dnd.set_title("Dendrogram — Merge History", fontsize=11)
                    ax_dnd.set_xlabel("Data Points"); ax_dnd.set_ylabel("Distance")
                    ax_dnd.axhline(link_matrix[-hc_k_used+1, 2], color=ACCENT3,
                                   linestyle="--", linewidth=1.5, label=f"Cut at k={hc_k_used}")
                    ax_dnd.legend(fontsize=8, framealpha=0.2, labelcolor=TEXT_CLR)
                    apply_theme(fig_dnd, ax_dnd); fig_dnd.tight_layout(); st.pyplot(fig_dnd); plt.close()

                st.markdown("#### Cluster Profiles (Feature Means)")
                prof_df = pd.DataFrame(X_hc_plot, columns=unsup_feats)
                prof_df["__cluster__"] = hc_labels
                prof_means = prof_df.groupby("__cluster__")[unsup_feats].mean().round(3)
                prof_means.index = [f"Cluster {i+1}" for i in prof_means.index]
                st.dataframe(prof_means.T, use_container_width=True)

                st.markdown(f"""
<div class="cluster-info-card">
  <div class="ci-header">📊 What the results mean</div>
  <div class="ci-body">
    Hierarchical clustering merged your data bottom-up into <strong>{hc_k_used} clusters</strong>
    using <strong>{hc_linkage} linkage</strong>.
    The dendrogram on the right shows <em>how</em> clusters were formed — taller vertical bars
    mean bigger jumps in distance when merging (natural cluster boundaries).
    {'Silhouette score ' + f'{hc_sil:.2f}' + ' → ' + ('strong' if hc_sil > 0.5 else 'moderate' if hc_sil > 0.25 else 'weak') + ' separation.' if not np.isnan(hc_sil) else ''}
  </div>
</div>
""", unsafe_allow_html=True)
            ...
            st.markdown('<div class="module-banner" style="border-left-color:#e0a844;">'
                        '<div class="mod-label" style="color:#e0a844;">⬡ Algorithm · t-SNE · sklearn</div>'
                        '<div class="mod-title">t-SNE — 2D Dimensionality Reduction</div>'
                        '<div class="mod-sub">Reveals hidden clusters and structure by projecting high-dimensional data into 2D</div>'
                        '</div>', unsafe_allow_html=True)

            st.markdown("""
<div class="problem-explain-box" style="border-left-color:#e0a844;">
  <strong>How t-SNE works:</strong><br>
  t-SNE (t-distributed Stochastic Neighbour Embedding) compresses your data from many dimensions
  down to just 2, preserving the <em>neighbourhoods</em> — points that are similar in the
  original space will appear close together in the 2D plot. This makes hidden clusters and
  structures <em>visible to the human eye</em>.
  <br><br>
  ⚠ t-SNE is for <em>visualisation only</em> — distances in the 2D plot are not directly
...
""", unsafe_allow_html=True)
            ...
            with ts1:
                ts_perp = st.slider("Perplexity", 5, 80, 30, 5, key="ts_perp",
                                    help="Balances local vs global structure. Roughly = neighbourhood size.")
            with ts2:
                ts_iter = st.slider("Iterations", 250, 1000, 500, 250, key="ts_iter")
            with ts3:
                ts_lr = st.slider("Learning rate", 10, 1000, 200, 10, key="ts_lr")
            with ts4:
                ts_max_rows = st.slider("Max rows", 100, min(2000, len(data)), min(500, len(data)), 100, key="ts_rows")

            colour_by_opts = ["None (single colour)"] + all_cols
            ts_colour = st.selectbox("Colour points by column (optional)", colour_by_opts, key="ts_colour")
            ...
            if run_tsne:
                ...
                if len(X_ts) < ts_perp:
                    st.error(f"Perplexity ({ts_perp}) must be < number of data points ({len(X_ts)}). Reduce perplexity or increase max rows.")
                else:
                    with st.spinner("Running t-SNE (this may take a moment)…"):
                        tsne_model = TSNE(n_components=2, perplexity=min(ts_perp, len(X_ts)-1),
                                                    max_iter=ts_iter, learning_rate=ts_lr,
                                                           random_state=42, init="pca")
                        X_ts_2d = tsne_model.fit_transform(X_ts)
                        st.session_state["_tsne_2d"]      = X_ts_2d
                        st.session_state["_tsne_rows"]    = ts_max_rows
                        st.session_state["_tsne_colour"]  = ts_colour
                        st.session_state["_tsne_perp"]    = ts_perp

            if st.session_state.get("_tsne_2d") is not None:
                X_ts_2d     = st.session_state["_tsne_2d"]
                n_ts        = len(X_ts_2d)
                ts_colour   = st.session_state["_tsne_colour"]
                ts_perp_val = st.session_state["_tsne_perp"]
                ...
                if ts_colour != "None (single colour)" and ts_colour in data.columns:
                    ...
                    if colour_col.dtype == object or colour_col.nunique() <= 15:
                        ...
                        for i, val in enumerate(unique_vals):
                            mask = colour_col == val; clr = CLUSTER_PALETTE[i % len(CLUSTER_PALETTE)]
                            ax_ts.scatter(X_ts_2d[mask,0], X_ts_2d[mask,1],
                                          s=40, color=clr, alpha=0.75, edgecolors="none",
                                          label=str(val))
                        ax_ts.legend(title=ts_colour, fontsize=8, framealpha=0.2,
                                     labelcolor=TEXT_CLR, facecolor=AXES_BG, edgecolor=GRID_CLR)
                    else:
                        norm_vals = colour_col.fillna(colour_col.median()).values
                        sc = ax_ts.scatter(X_ts_2d[:,0], X_ts_2d[:,1], s=40, c=norm_vals,
                                           cmap="plasma", alpha=0.75, edgecolors="none")
                        plt.colorbar(sc, ax=ax_ts, shrink=0.8, label=ts_colour)
                else:
                    ax_ts.scatter(X_ts_2d[:,0], X_ts_2d[:,1], s=40, color=ACCENT1,
                                  alpha=0.65, edgecolors="none")

                ax_ts.set_xlabel("t-SNE Dimension 1"); ax_ts.set_ylabel("t-SNE Dimension 2")
                ax_ts.set_title(f"t-SNE 2D Projection — {n_ts} points  (perplexity={ts_perp_val})", fontsize=12)
                apply_theme(fig_ts, ax_ts); fig_ts.tight_layout(); st.pyplot(fig_ts); plt.close()

                st.markdown("""
<div class="cluster-info-card">
  <div class="ci-header">💡 How to read this chart</div>
  <div class="ci-body">
    Each dot represents one row in your dataset. <strong>Dots close together have similar features</strong>.
    Visible clusters (blobs) suggest natural groupings in your data. If you coloured points by a
    column, matching colours appearing in the same blob confirm that column correlates with the
    data structure. Try running <strong>K-Means or DBSCAN</strong> on the same features to
    formally label these groups.
  </div>
</div>
""", unsafe_allow_html=True)

                if st.session_state.get("_km_labels") is not None and len(st.session_state["_km_labels"]) == n_ts:
                    st.markdown("#### t-SNE coloured by K-Means cluster labels")
                    km_lbl_ts = st.session_state["_km_labels"]
                    fig_ts2, ax_ts2 = plt.subplots(figsize=(9, 6))
                    for cid in np.unique(km_lbl_ts):
                        mask = km_lbl_ts == cid; clr = CLUSTER_PALETTE[cid % len(CLUSTER_PALETTE)]
                        ax_ts2.scatter(X_ts_2d[mask,0], X_ts_2d[mask,1],
                                       s=40, color=clr, alpha=0.8, edgecolors="none", label=f"K-Means G{cid+1}")
                    ax_ts2.set_xlabel("t-SNE 1"); ax_ts2.set_ylabel("t-SNE 2")
                    ax_ts2.set_title("t-SNE + K-Means Labels Overlay", fontsize=11)
                    ax_ts2.legend(fontsize=8, framealpha=0.2, labelcolor=TEXT_CLR, facecolor=AXES_BG, edgecolor=GRID_CLR)
                    apply_theme(fig_ts2, ax_ts2); fig_ts2.tight_layout(); st.pyplot(fig_ts2); plt.close()

        elif unsup_algo == "Apriori — Association Rules":
            st.markdown('<div class="module-banner" style="border-left-color:#d05090;">'
                        '<div class="mod-label" style="color:#d05090;">⬡ Algorithm · Apriori · Manual</div>'
